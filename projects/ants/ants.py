...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def nearest(place, in_range, distance):
            if place.entrance == None or place == hive:
                return None 
            elif in_range(distance) and place.bees != []:
                return random_or_none(place.bees)
            else:
                return nearest(place.entrance, in_range, distance + 1)
        return nearest(self.place, in_range, 0)
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.rest_turns > 0:
            self.rest_turns -= 1
        elif self.eat_bee(random_or_none(self.place.bees)):
            self.rest_turns = self.time_to_digest
        # END Problem 6
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn but has a large armor"""
    name = "Wall"
    implemented = True
    food_cost = 4
...
class ScubaThrower(ThrowerAnt):
    ...
    def reduce_armor(self, count):
        if self.place is not Water:
            ThrowerAnt.reduce_armor(count)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        self.armor = armor
        self.is_true = QueenAnt.is_true_next
        if QueenAnt.is_true_next:
            QueenAnt.is_true_next = False
    ...
    def action(self, colony):
        ...
        def get_entrance(place):
            return place.entrance 

        def get_exit(place):
            return place.exit

        def double_damage(place, fns):
            for fn in fns:
                current_place = fn(place) 
                while current_place != None and current_place != place:
                    if current_place.ant != None:
                        if current_place.ant not in buffed_ants:
                            current_place.ant.damage *= 2
                            buffed_ants.append(current_place.ant)
                        if current_place.ant.is_container and current_place.ant.contained_ant and \
                           current_place.ant.contained_ant not in buffed_ants:
                            current_place.ant.contained_ant.damage *= 2
                            buffed_ants.append(current_place.ant.contained_ant)
                    current_place = fn(current_place)

        if self.is_true:
            ScubaThrower.action(self, colony)
            double_damage(self.place, [get_exit])
        else:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    bee.effect = effect(bee.action, bee)
